...
Page({
 ...
 pigai: function (e) {
  ...
  console.log(this.data.zuoyefabulist[e.currentTarget.dataset.id]);
     ...
     console.log(id);
     wx.navigateTo({
       url: '../zuoyepigai/zuoyepigai?id=' + id,
     })

},
 /* 删除作业 */
 del: function (e) {
  console.log("点击后de作业发布全部Info：")
  console.log(this.data.zuoyefabulist[e.currentTarget.dataset.id]);
  console.log("点击后的this.data作业内容：" +this.data.zuoyefabulist[e.currentTarget.dataset.id].zuoyeneirong);
  wx.request({
    url: 'http://localhost:8080/deletezuoyefabu/',
    method:"post",
    data:{
      id:this.data.zuoyefabulist[e.currentTarget.dataset.id].id
    },  
    success:res=>{
      console.log(res.data);
      if(res.statusCode=="200"){       
       this.getZuoyefabulist()
      }else{
        console.log('删除失败')
      }
    }
