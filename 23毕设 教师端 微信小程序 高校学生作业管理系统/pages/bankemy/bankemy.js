...
//1.获取当前 操作的元素 的标识e.currentTarget.dataset.index
 ...
 //调用get的方法 更新
  ...
  getBankeData:function(){  
   wx.request({
    url: 'http://localhost:8080/searchbanketeaid',
    method:"post",
    data:{
      teacherid:app.globalData.userInfo.id
    },  
  ...
  /* 删除item */
  delbanke: function (e) {
    // this.data.bankemyList.splice(e.target.id.substring(3), 1);
    // // 更新data数据对象  
    // if (this.data.bankemyList.length > 0) {
    //   this.setData({
    //     bankemyList: this.data.bankemyList
    //   })
    //   wx.setStorageSync('bankemyList', this.data.bankemyList);
    // } else {
    //   this.setData({
    //     bankemyList: this.data.bankemyList
    //   })
    //   wx.setStorageSync('bankemyList', []);
    // }
    console.log("点击后的全局变量班课名：")
    console.log(app.globalData.bankeInfo);
    console.log("点击后的this.data班课名：" +this.data.list[e.currentTarget.dataset.id].bname);
